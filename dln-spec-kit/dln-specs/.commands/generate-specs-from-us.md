...
- Load `rules/coding-standards.mdc` (always), plus the technology-specific file: `rules/laravel-standards.mdc`, `rules/python-standards.mdc`, or `rules/ci4-standards.mdc` for backend specs; `rules/frontend-standards.mdc` for frontend specs.
...
**Naming convention**: `YYYYMMDD-[us-id]-spec[N]-[kebab-case-description].md`

Where:
- `YYYYMMDD` is today's date.
- `[us-id]` is the US identifier extracted from the source US filename (e.g. `us001` from `20260512-us001-*.md`).
- `[N]` is the **1-based sequential position** of this spec within the User Story checklist. **N MUST start at 1. N=0 is strictly forbidden.**
- `[kebab-case-description]` is a short (2–5 words), lowercase, hyphen-separated description of what this spec implements.

Valid examples:
- `20260514-us001-spec1-laravel-bootstrap.md`
- `20260514-us001-spec2-tailwind-brand-config.md`
- `20260514-us001-spec3-playwright-functional-tests.md`

**NEVER produce filenames like these:**
- `20260514-us001-spec0-tailwind-brand-config.md` ← N=0 is forbidden
- `20260514-us001-playwright-functional-tests.md` ← missing `spec[N]` segment
- `20260514-user-authentication-api-login-endpoint.md` ← missing `[us-id]` and `spec[N]`

**Every checklist item — including `Playwright Functional Tests` — MUST generate a numbered spec file following this exact convention. No item is exempt.**
...
- Review all changes made and update the relevant files in `rules/`:
  ...
  - Backend patterns → `rules/laravel-standards.mdc`, `rules/python-standards.mdc`, or `rules/ci4-standards.mdc` as applicable
...
- Reminder: use `/develop-spec` to implement each spec, and `/complete-spec` when done.

---

## Standards Applied

- **Backend (Laravel)**: `rules/coding-standards.mdc` + `rules/laravel-standards.mdc` — Service layer, thin controllers, Eloquent, Passport auth, Jobs for async, PHPUnit TDD, branch naming `feature/YYYYMMDD_*`.
- **Backend (CI4)**: `rules/coding-standards.mdc` + `rules/ci4-standards.mdc` — Simple MVC, one view per route, no service layer.
- **Backend (Python)**: `rules/coding-standards.mdc` + `rules/python-standards.mdc` — Python 3.12, unittest, type hints, logging.
